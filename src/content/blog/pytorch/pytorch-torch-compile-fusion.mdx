...
算子融合减少的是**GPU 端**的开销（减少显存读写）。而 **CUDA Graphs** 减少的是 **CPU 端**的开销：
...
CUDA Graphs 要求**输入 shape 固定**——记录的 tensor 形状和回放时必须一致，而 LLM 的序列长度是动态变化的，看起来不能直接用，
但是目前工业界往往**采用 padding 绕过这个限制**：
...
[torch.compile 底层在做什么？从 Triton 代码生成到手写 CUDA kernel →](/blog/pytorch/pytorch-torch-compile-internals)
